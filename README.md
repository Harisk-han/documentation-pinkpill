...
2. Botpress checks Supabase on every message to verify the user's subscription status.
...
5. If the user is a confirmed premium subscriber (verified via Supabase), full access is restored immediately.
6. Pinky detects the user's dating stage and intent.
7. The user is routed into the appropriate mode (Vetting, Coaching, Exit, or Swirling).
8. Responses are delivered based on the user's current Knowledge Tier (1, 2, or 3).
9. Milestones are tracked to progressively unlock deeper content.
...
Pinky uses a **free trial + premium subscription** model managed via Supabase.

| Access Level | Duration | Behavior |
|---|---|---|
| **Free Trial** | 1 week (one-time, no reset) | Full access to all features |
| **Trial Expired** | After 1 week, no payment | Bot sends upgrade prompt — no advice delivered |
| **Premium** | Paid subscriber (verified via Supabase) | Full access to all tiers immediately |

**Key Rules:**
- The free trial is one-time only — it does not reset.
- Botpress checks Supabase on **every message** to verify subscription status.
- When Supabase confirms payment, the user is immediately treated as a premium subscriber with full Tier access.
- Premium subscriber data (Name + Email) is sent via webhook to n8n, which saves it in Constant Contact.
...
The entry point of the bot. Handles welcome, subscription check, dating stage detection, and routes the user to the correct mode flow.
...
### 4. Web Integration
Documents how Pinky is embedded and deployed on the web, including widget configuration, Supabase subscription checking, and n8n webhook setup.
...
Pinky uses a three-tier progressive content system. All tiers are accessible to free trial users and premium subscribers. Trial-expired users must upgrade before any tier content is delivered.
